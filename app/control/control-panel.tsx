...
export default function Dashboard() {
  ...
  const loadStatus = useCallback(async (mode: "connect" | "refresh") => {
    ...
    try {
      ...
      setError(
        `${reason} Start the local service at ${API_LABEL}, then retry.`,
      );
    } finally {
      setAction(null);
    }
  ...
  async function saveProxies(mode: ProxyMode) {
    ...
    try {
      ...
      if (caught instanceof TypeError) {
        ...
        setCounts({ static: null, rotating: null, active: null });
        setError(
          `${label} proxy list was not saved. Start the local service at ${API_LABEL}, reconnect, then try again. Draft kept.`,
        );
      ...
      <aside className="status-rail" aria-label="Local service status">
        ...
        <div className="rail-actions">
          ...
          <p className="permission-note">
            This UI is served by the loopback service. Controls and proxy drafts stay on this PC. Existing proxy values are
            never displayed.
          </p>
        </div>
      </aside>
          ...
          <details className="proxy-files">
            ...
            <div className="proxy-files-body">
              <p>
                ...
                to {API_LABEL} and are never persisted in this browser.
              </p>
