...
async def test_save_and_configuration_are_safe_while_launch_uses_its_snapshot(
    ...
    static_file.write_text("old.test:8001\n", encoding="utf-8")
    ...
    async with app.router.lifespan_context(app):
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            launching = asyncio.create_task(
                client.post("/browsers", json={"static_windows_per_proxy": 1})
            )
            await launch_entered.wait()
            try:
                saved = await client.post(
                    "/configuration/proxies",
                    json={"mode": "static", "proxies": "new-one.test:9001\nnew-two.test:9002\n"},
                )
    ...
    assert saved.json() == {"mode": "static", "count": 2}
    assert configuration.json()["static_proxy_count"] == 2
    assert launched.status_code == 200
    assert launcher.proxies == [{"server": "http://old.test:8001"}]
    assert static_file.read_text(encoding="utf-8") == "new-one.test:9001\nnew-two.test:9002\n"
